...
<script>
    ...
    $(document).on('submit', '.checkout-form', function(e) {
        ...
        if ($('.checkout-form input[name=payment_method]:checked').val() == 'cod') {
            var priceString = $('#g-total').text();
            let numericString = priceString.replace(/[^\d.]/g, '');
            let priceFloat = parseFloat(numericString);
            if(priceFloat >= 10000){
                notifyMe('error', 'Your Amount is Greater Then 10K Please Select Online Payment');
                e.preventDefault();
                return false;
            }
        }
