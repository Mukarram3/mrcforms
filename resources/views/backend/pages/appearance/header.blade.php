...
@section('contents')
    <section class="tt-section pt-4">
        <div class="container">
            <div class="row mb-3">
{{--                navigation color--}}
                <form action="{{ route('admin.nav.color') }}" method="POST" enctype="multipart/form-data"
                      class="">
                    @csrf
                    <div class="card mb-4" id="section-1">
                        <div class="card-body">
                            <h5 class="mb-4">{{ localize('Navigation Color') }}</h5>

                            <div class="mb-3">
                                <label for="topbar_welcome_text"
                                       class="form-label">{{ localize('Select Color') }}</label>

{{--                                <input type="color" name="background_color" id="topbar_welcome_text"--}}
...
{{--                                       value="{{ getSetting('topbar_welcome_text') }}">--}}
                                ...
                                <label for="topbar_welcome_text"
                                       class="form-label">{{ localize('Enter Color Code') }}</label>

                                <input type="text" name="color_code" id="topbar_welcome_text"
                                       class="form-control">
                            </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-xl-9 order-2 order-md-2 order-lg-2 order-xl-1">
                    <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="card mb-4" id="section-2">
                            <div class="card-body">
                                <h5 class="mb-4">{{ localize('Invoice Logo') }}</h5>

                                <div class="mb-3">
                                    <label class="form-label">{{ localize('Invoice Logo') }}</label>
                                    ...
                                    <div class="tt-image-drop rounded">
                                        ...
                                        <div class="tt-product-thumb show-selected-files mt-3">
                                            <div class="avatar avatar-xl cursor-pointer choose-media"
                                                 data-bs-toggle="offcanvas" data-bs-target="#offcanvasBottom"
                                                 onclick="showMediaManager(this)" data-selection="single">
                                                <input type="hidden" name="invoice_logo"
                                                       value="{{ getSetting('invoice_logo') }}">
                                                <div class="no-avatar rounded-circle">
                                                    <span><i data-feather="plus"></i></span>
                                                </div>
                                            </div>
                                ...
                                <div class="mb-3">
                                    <label for="invoice_whatsapp_number"
                                           class="form-label">{{ localize('Invoice WhatsApp Number') }}</label>
                                    <input type="hidden" name="types[]" value="invoice_whatsapp_number">
                                    <input type="text" name="invoice_whatsapp_number" id="invoice_whatsapp_number"
                                           class="form-control" placeholder="+80 157 058 4567"
                                           value="{{ getSetting('invoice_whatsapp_number') }}">
                                </div>
