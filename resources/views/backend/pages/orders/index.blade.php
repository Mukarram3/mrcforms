                                ...
                                <div class="row justify-content-between g-3">
                                    <div class="col-auto flex-grow-1">
                                        <div class="input-group mb-3">
                                            ...
                                            @endif
                                            <input type="text" class="form-control" placeholder="{{ localize('Search by code/phone') }}"
                                                name="code"
                                                @isset($searchCode)
                                                value="{{ $searchCode }}"
                                                @endisset>
                                        ...
                                        <div class="tt-search-box">
                                            <div class="input-group">
                                                ...
                                                <input class="form-control rounded-start w-100" type="text"
                                                       ...
                                                       placeholder="{{ localize('Search by name/email') }}"
                                                       @isset($searchKey)
                                                           value="{{ $searchKey }}"
                                                        @endisset>
                                            </div>
...
{{--                                    @endif--}}
                                    ...
                                    <div class="col-auto">
                                        <a href="{{ route('admin.order.direct.create') }}" class="btn btn-primary">
                                            {{--                                            <i data-feather="search" width="18"></i>--}}
                                            New Order
                                        </a>
                                    </div>
...
{{--                                    <th class="text-center">{{ localize('S/L') }}--}}
                                    <th class="text-center">{{ localize('Order ID') }}
                                    </th>
                                    <th>{{ localize('Order Code') }}</th>
                                    <th data-breakpoints="xs sm md">{{ localize('Customer') }}</th>
                                    <th data-breakpoints="xs sm md">{{ localize('Email') }}</th>
                                    ...
                                    <th data-breakpoints="xs sm">{{ localize('Payment Type') }}</th>
{{--                                    @if (count($locations) > 0)--}}
                                        <th data-breakpoints="xs sm">{{ localize('Store Name') }}</th>
{{--                                    @endif--}}
{{--                                    <th data-breakpoints="xs sm">{{ localize('Remarks Box') }}</th>--}}
...
{{--                                                    <span class="text-muted fs-sm">{{ optional($order->user)->phone ?? '-' }}</span>--}}
                                                    <span class="text-muted fs-sm">{{ optional($order->orderGroup)->phone_no ?? '-' }}</span>
                                                </div>
                                            </div>
                                        </td>

                                        <td>
                                            <span class="badge rounded-pill text-capitalize bg-secondary">{{ optional($order->user)->email }}</span>

                                        </td>

                                        <td>
{{--                                            <span class="fs-sm">{{ date('d M, Y', strtotime($order->created_at)) }}</span>--}}
                                            <span class="fs-sm">{{ date('d M, Y H:i:s', strtotime($order->created_at)) }}</span>
                                        ...
                                        <td>
                                            ...
                                            <span class="badge rounded-pill text-capitalize bg-secondary">{{ $order->orderGroup->payment_method }}</span>
                                        </td>
                                        <td>
                                            <span class="badge rounded-pill text-capitalize bg-secondary">
                                                {{ $order->branch_name }} </span>
                                        </td>
{{--                                        Remarks Box--}}

{{--                                        <td>--}}
{{--                                            <span class="badge rounded-pill text-capitalize bg-secondary">--}}
{{--                                              {{ $order->orderGroup->remarks_box }}  </span>--}}
{{--                                        </td>--}}
                                        ...
                                        {{--                                        <td>--}}
                                        {{--                                            <span class="badge rounded-pill text-capitalize bg-secondary">--}}
                                        {{--                                                                                      {{ $order->orderGroup->phone_no }}  </span>--}}
                                        {{--                                        </td>--}}
                                        {{--                                       END PHONE NUMBER--}}
