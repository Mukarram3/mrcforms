        ...
        @endphp
            ...
            <div class="dropdown tt-tb-dropdown">
                <button type="button" class="btn btn-light px-3 py-1" data-bs-toggle="dropdown" aria-expanded="false">
                    {{ $branch->name }}<i data-feather="chevron-down" class="ms-1"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-end shadow">


                    @foreach (\App\Models\Branch::where('status', "active")->get() as $key => $branch)
                        <a class="dropdown-item" href="javascript:void(0);" onclick="changeLocation(this)"
                            data-location="{{ $branch->id }}">
                            <i data-feather="map-pin" class="me-2"></i>{{ $branch->name }}
                        </a>
                    @endforeach
                </div>
            </div>
